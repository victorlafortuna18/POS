...
<div class="container-fluid">
	...
	@else
		@foreach($pedidosPendientes as $pedidopendiente)
			<?php $conteoPedidos++; ?>
			<?php $lastpedidoid = $pedidopendiente->id; ?>
		@endforeach
	@endif
	...
	<!-- Pedidos -->
	<a href="{{ url('adminoperador/ordenes') }}" class="row conteo btn-warning" data-toggle="tooltip">
	...
	<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-default">
				<input type="hidden" name="lastpedidoid" id="lastpedidoid" value="{{$lastpedidoid}}">
				<input type="hidden" name="idsucursal" id="idsucursal" value="{{ $sucursalID }}">
				<input type="hidden" name="cuentaid" id="cuentaid" value="{{ $id }}">
					...
					<div class="panel-body">
						...
						<div class="list-group">
							<table id="tablaPedidos" class="display" cellspacing="0" width="100%">
								...
								<tr class="{{$pedido->id}}">
									<td>
										<a href="{{ url('adminoperador/consultorder/'.$pedido->tracking_id) }}" id="itemPedido" name="{{$pedido->id}}">{{$pedido->orden}}</a> 
									...
									<td><span class="timeElapsed" data-comenzar="{{ $pedido->timeCreated }}" data-id="{{ $pedido->id }}" style="float:right;">{{ $diferencia_real }}</span></td>
...
<script>
	$(document).ready(function(){
		...
		var tabledata = $('#tablaPedidos').DataTable({
		...
		//Search for new pedidos and add to the datatable
		function refreshPedidos(){
			
			var dominio = document.domain;
			var asunto = 'searchnewpedidos';
			var IDs = "";
			var dato = "";
			$(".timeElapsed").each(function(index){
				dato = dato + $(this).attr('data-id') + ',';
			});
					...
					$.each(datos, function( index, value ) {
					  //alert( value['cliente'] );
						if(document.getElementById('cuentaid').value == value['cuentaID']){
							if(document.getElementById('idsucursal').value == value['localidadID']){
								
								contador++;
								if(parseInt(document.getElementById('lastpedidoid').value) < parseInt(value['id'])){
									
									var tracking_id = value['tracking_id'];
									var orden = "<a href=\"consultorder/"+ tracking_id +"\" id=\"itemPedido\" name=\""+ value['id'] +"\">"+ value['orden'] +"</a>";
									var cliente = "-";	
									var identificacion = "-";	
									var direccion = "-";
									var total = value['total'];
									var mensajero = "-";
									var tiempo = "<span class=\"timeElapsed\" data-comenzar=\""+ value['timeCreated'] +"\" data-id=\""+ value['id'] +"\" style=\"float:right;\">...</span>";
									var estado = "<span class=\"estado\" id=\""+ value['id'] +"\" style=\"font-weight:bold;font-size:14px;\">...</span>";
									...
									document.getElementById('lastpedidoid').value = value['id'];
