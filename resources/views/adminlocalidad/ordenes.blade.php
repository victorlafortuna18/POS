...
<div class="container-fluid">
	...
	<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-default">
				...
				<input type="hidden" name="rol" id="rol" value="adminlocalidad">
									...
									@if($orden->cuentaid == $cuentaID)
										@if($orden->sucursalid == $idSucursal)
											<tr class="{{ $orden->id }}" data-url="edit-orden" data-id="{{ $orden->id }}" data-ordenid="{{ $orden->ordenid }}">
												...
												<td>
													<a href="#" id="eliminarOrden" class="eliminarOrden" data-id="{{ $orden->id }}" data-ordenid="{{ $orden->ordenid }}" data-clientnombre="{{ $orden->cliente }}" style="font-weight:bold;float:right;position:relative;top:-3px;"><i class="fa fa-trash" aria-hidden="true"></i></a>
												</td>
	...
	<!-- Modal -->
	<div id="modalEliminarOrden" style="display:none;position:absolute;top:0px;left:0px;width:100%;height:100%;background:rgba(0,0,0,0.7);box-sizing:border-box;z-index:1000000;">
		<div class="modal-dialog" role="document" style="margin:0px auto;margin-top:100px;">
			<div class="modal-content">
				<div class="modal-header">
					<button  id="closeModalEliminarOrden" type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
					<h4 class="modal-title" id="myModalLabel"><i class="fa fa-exclamation-triangle fa-2x"></i> Eliminar Orden</h4>
				</div>
				<form id="formEliminarOrden" action="ordenes" method="post">
					<div class="modal-body">
							Desea eliminar la orden <span id="modalEliminarOrdenTitle"></span>?
							<input type="hidden" name="_token" value="{{ csrf_token() }}">
							<input type="hidden" name="asunto" value="eliminarOrden">
							<input type="hidden" id="ordenid" name="ordenid" value="-1">
							<input type="hidden" id="clientnombre" name="clientnombre" value="-1">
							<input id="id" type="hidden" name="id" class="text" value="">
						
					</div>
					<div class="modal-footer">
						<a id="CancelarEliminarOrden" type="button" class="btn btn-default" data-dismiss="modal">Cancelar</a>
						<input type="submit" id="borrarOrden" type="button" value="Eliminar" class="btn btn-success" data-dismiss="modal">
					</div>
				</form>
			</div>
		</div>
			...
			<div class="modal-content">
				...
				<form id="formAsignarOrden" action="ordenes" method="post">
					<div class="modal-body">
							...
							<button type="button" id="geoButton" class="btn btn-link" data-def="-1" style="position:absolute;top:10px;right:10px;"><i class="fa fa-map-marker" aria-hidden="true"></i></button>
					</div>
							...
							@else
								@foreach($mensajeros as $mensajero)
									@if($mensajero->EstatusActual == 0)
										<a href="#" style="display:block;padding:3px;margin-bottom:3px;border-left: 5px solid #A9F5A9;font-weight:bold;box-sizing:border-box;" name="{{ $mensajero->id }}" data-mensajeroID="{{ $mensajero->id }}" data-estatus="0">{{ $mensajero->nombreUsuario }}</a>
									@endif
								@endforeach
								@foreach($mensajeros as $mensajero)
									@if($mensajero->EstatusActual == 1)
										<a href="#" style="display:block;padding:3px;margin-bottom:3px;border-left: 5px solid #FFEC7A;font-weight:bold;box-sizing:border-box;" name="{{ $mensajero->id }}" data-mensajeroID="{{ $mensajero->id }}" data-estatus="1">{{ $mensajero->nombreUsuario }}</a>
									@endif
								@endforeach
								@foreach($mensajeros as $mensajero)
									@if($mensajero->EstatusActual == 2)
										<a href="#" style="display:block;padding:3px;margin-bottom:3px;border-left: 5px solid #F78181;font-weight:bold;box-sizing:border-box;" name="{{ $mensajero->id }}" data-mensajeroID="{{ $mensajero->id }}" data-estatus="2">{{ $mensajero->nombreUsuario }}</a>
									@endif
								@endforeach
							@endif
						</div>
...
<script>
	$(document).ready(function(){
						...
						if(document.getElementById('cuentaid').value == value['cuentaid']){
							if(document.getElementById('idsucursal').value == value['sucursalid']){
								...
								if(parseInt(document.getElementById('lastordenid').value) < parseInt(value['id'])){
									...
									var borrarBoton = "<a href=\"#\" id=\"eliminarOrden\" class=\"eliminarOrden\" data-id=\"" + value['id'] + "\" data-ordenid=\""+ value['ordenid'] +"\" data-clientnombre=\"" + value['cliente'] + "\" style=\"font-weight:bold;float:right;position:relative;top:-3px;\"><i class=\"fa fa-trash\" aria-hidden=\"true\"></i></a>";
		...
		//Functions modalEliminarMensajero
			...
			//$('.eliminarMensajero').click(function(){
				event.preventDefault();
				$('#formEliminarOrden #id').val($(this).attr('data-id'));
				$('#formEliminarOrden #ordenid').val($(this).attr('data-ordenid'));
				$('#formEliminarOrden #clientnombre').val($(this).attr('data-clientnombre'));
				$('#formEliminarOrden #modalEliminarOrdenTitle').text($(this).attr('data-ordenid'));
				$('#modalEliminarOrden').fadeToggle('fast');
				
				return false;
			});
			$('#closeModalEliminarOrden, #CancelarEliminarOrden').click(function(){
				event.preventDefault();
				$('#modalEliminarOrden').fadeToggle('fast');
			});
			$('#borrarOrden').click(function(){
				var id = $('#formEliminarOrden #id').val();
				
				if(id == ''){
					alert('No se ha podido identificar esta orden');
					return false;
				}
			});
		//***************************************/
		...
		//Functions modalAsignarOrden
			...
			//$('.itemOrden').click(function(){
				...
				$('#geoButton').removeClass("btn-danger");
				$('#geoButton').removeClass("btn-success");
				$('#geoButton').removeClass("btn-primary");
				$('#geoButton').addClass("btn-link");
	...
	function guardarClienteOrdenes(){
		...
		$('#geoButton').removeClass("btn-link");
		$('#geoButton').addClass("btn-danger");
		$('#geoButton').attr("data-def","1");
...
//******************************************************************************************************************************/
				...
				xhr.onreadystatechange = function() {
					if (xhr.readyState == 4 && xhr.status == 200) {
						...
						if(json.posts[1].values[0].posicionDefinitiva == 0) {
							...
							$('#geoButton').removeClass("btn-link");
							$('#geoButton').removeClass("btn-danger");
							$('#geoButton').addClass("btn-success");
							$('#geoButton').attr("data-def","0");
						}
	...
	//********************************************************************************/
		...
		$(document).on('click','#geoButton',function(){
			var dominio = document.domain;
			var asunto = 'switchPosicionDefinitiva';
			var defResult;
			
			var clienteid = $('#clienteID').attr('value');
			var def = $('#geoButton').attr('data-def');
			
			if(def == '1'){
				defResult = '0';
				//$('#geoButton').attr("data-def",defResult);
